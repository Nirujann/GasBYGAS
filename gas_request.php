...
<body class="g-sidenav-show  bg-gray-100">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $conn->prepare("SELECT available_stock FROM outlets WHERE outlet_id = ?");
        $stmt->bind_param("i", $outlet_id);
        $stmt->execute();
        $result = $stmt->get_result();
        $outlet = $result->fetch_assoc();

        if ($outlet['available_stock'] >= $quantity) {
            $stmt = $conn->prepare("INSERT INTO gas_requests (user_id, outlet_id, cylinder_type_id, quantity, token_number) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("iiiis", $user_id, $outlet_id, $cylinder_type, $quantity, $token);
            
            if ($stmt->execute()) {

                $request_id = $stmt->insert_id;
                
                send_notification($user_id, $request_id ,"Your gas request has been created. Token: " . $token, "email", $email);
    ...
    <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 "
        ...
        <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
            ...
            <ul class="navbar-nav">
                ...
                ?>
                <li class="nav-item">
                    <a class="nav-link  active" href="gas_request.php">
                        <div
                            class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
    ...
    <div class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
                ...
                <div class="container-fluid py-1 px-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                            ...
                            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Gas Request</li>
                        </ol>
                        <h6 class="font-weight-bolder mb-0">Gas Request</h6>
                    </nav>
            ...
            <div class="col-md-12 mt-4" style="display: flex;justify-content: center;">
                <div class="col-md-8">
                    <form id="gasRequestForm" method="POST" class="request-form">
                        <div class="card" style="padding: 50px;">
                            <div class="col-md-12" style="display: flex;justify-content: flex-end;">
                                <a href="my_requests.php"><button class="btn btn-primary">My Requests</button></a>
                            </div>
                            <div class="card-header">Add Gas Request</div>
                            <div class="form-group">
                                ...
                                <select name="outlet_id" class="form-select" required>
                                    ...
                                    $conn = connect_db();
                                    if ($conn) {
                                        $outlets = $conn->query("SELECT outlet_id, outlet_name FROM outlets WHERE status = 'active'");

                                        if ($outlets && $outlets->num_rows > 0) {

                                            while ($outlet = $outlets->fetch_assoc()) {
                                                echo "<option value='" . htmlspecialchars($outlet['outlet_id'], ENT_QUOTES, 'UTF-8') . "'>" . htmlspecialchars($outlet['outlet_name'], ENT_QUOTES, 'UTF-8') . "</option>";
                                            }
                                        } else {
                                            echo "<option>No active outlets available</option>";
                                        }
                                    } else {
                                        echo "<option>Error fetching outlets</option>";
                                    }
                                    ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="cylinder_type" class="form-select" required>
                                    <?php
                                    $types = $conn->query("SELECT type_id, name FROM cylinder_types WHERE status = 'active'");
